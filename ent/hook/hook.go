...
func (f GardenFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.GardenMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.GardenMutation", m)
	}
	return f(ctx, mv)
}
